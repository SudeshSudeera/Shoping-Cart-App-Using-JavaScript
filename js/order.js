...
const loadItemData=()=>{
    ...
    database.collection('item') // use with s => items
...
addToCart=()=>{
    ...
    let tBody = $('#tBody'); //document.getElementById('tBody');
    tBody.empty();
    let rowData = '';
    ...
    /* cart.forEach(e=>{
        const rowData = $('<tr>');
        rowData.append($('<td>')).text(e.item.description);
        rowData.append($('<td>')).text(e.qty);
        rowData.append($('<td>')).text(e.item.unitPrice);
        rowData.append($('<td>')).text(e.total);
